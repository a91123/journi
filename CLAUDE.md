...
- [x] **JSON 匯出／匯入**（2026-07-20 完成）— 首頁「00 TRIPS」旁加「匯出備份／匯入備份」兩個小連結。匯出：把 `tripsStore.trips` 整包 `JSON.stringify` 成 `journi-backup-{日期}.json` 觸發下載。匯入：讀檔 → 基本欄位型別檢查（`id`/`destination`/`startDate`/`endDate`/`days`/`itinerary`）→ `confirm()` 顯示筆數請使用者確認 → `tripsStore.importTrips()` 以 `id` 去重（已存在的旅程不覆蓋，只補上沒有的），完成後顯示「新增 N 趟，跳過 M 趟重複」
