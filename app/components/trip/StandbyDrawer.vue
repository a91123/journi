  ...
  <Teleport to="body">
    ...
    <div class="fixed bottom-0 left-0 right-0 z-50 max-w-2xl mx-auto lg:hidden">
      <Transition name="slide-up">
        <div v-if="showDrawer" class="bg-paper-raised rounded-t-2xl shadow-2xl max-h-[65vh] flex flex-col">
          <div class="flex items-center justify-between px-5 py-3.5 border-b border-stub/70 flex-shrink-0">
            <span class="font-display font-bold text-ink text-sm">備用清單</span>
            <button @click="showDrawer = false" class="text-ink-faint hover:text-ink-soft text-xl leading-none">×</button>
          </div>
          <div class="overflow-y-auto flex-1 px-4 py-3 space-y-2">
            <div v-if="standbyItems.length === 0" class="text-center py-10 text-ink-faint/50">
            ...
            <div
              ...
              :key="item.id"
              class="bg-paper rounded-xl overflow-hidden"
            >
              <div class="flex items-start gap-3 py-3 px-3.5">
                <span class="text-lg mt-0.5 flex-shrink-0">{{ categoryEmoji(item.category) }}</span>
                <div class="flex-1 min-w-0">
                  <p class="font-semibold text-ink text-sm">{{ item.name }}</p>
                  <p v-if="item.duration" class="text-xs text-ink-faint mt-0.5 font-mono">⏱ 約 {{ item.duration }} 小時</p>
                </div>
                <div class="flex items-center gap-1.5 flex-shrink-0">
                  <button
                    @click="expandingId = expandingId === item.id ? null : item.id"
                    class="text-xs bg-airmail-red hover:bg-airmail-red/90 text-paper-raised px-3 py-1.5 rounded-lg font-medium transition-colors"
                  >排入</button>
                  <button
                    @click="tripsStore.removeFromStandby(trip.id, item.id)"
                    class="text-ink-faint/50 hover:text-airmail-red transition-colors text-lg leading-none"
                  >×</button>
                </div>
              </div>
              <!-- Day 選擇器 -->
              <div v-if="expandingId === item.id" class="px-3.5 pb-3 flex flex-wrap gap-2">
                <button
                  ...
                  class="text-xs px-3 py-1.5 rounded-lg font-medium transition-colors bg-paper-raised border border-stub hover:border-airmail-blue hover:text-airmail-blue text-ink-soft"
      ...
      <button
        ...
        class="w-full bg-ink hover:bg-ink/90 transition-colors text-paper py-3.5 flex items-center justify-center gap-2"
      >
        ...
        <span class="text-paper/50 text-xs ml-1">{{ showDrawer ? '▼' : '▲' }}</span>
      </button>
    </div>
  </Teleport>
</template>
